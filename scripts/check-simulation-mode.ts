...
async function checkSimulationMode() {
  ...
  try {
    ...
    console.log(
      `Simulation Mode: ${config.simulation_mode ? "✅ ON" : "❌ OFF"}`
    );
    console.log(
      `Current Race Time: ${config.current_race_time_sec || 0} seconds`
    );
    console.log(
      `Simulation Start: ${config.simulation_start_time || "NOT SET"}`
    );
    console.log("─".repeat(50));
    ...
    const leaderboardRes = await fetch(
      `${API_URL}/api/race/leaderboard?filter=overall`
    );

    if (leaderboardRes.ok) {
      const leaderboard = await leaderboardRes.json();
      console.log(
        `✅ Leaderboard OK: ${leaderboard.entries?.length || 0} runners`
      );
    } else {
