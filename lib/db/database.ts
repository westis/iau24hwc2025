...
export async function insertRunner(
  runner: Omit<Runner, "performanceHistory">
): Promise<number> {
...
export async function updateRunner(
  ...
  if (result.rowCount === 0) {
    console.error(
      `updateRunner: WARNING - No rows updated for entry_id ${entryId}`
    );
  }
}
...
export async function getRunnerByEntryId(
  entryId: string
): Promise<Runner | null> {
  const db = getDatabase();
  const result = await db.query("SELECT * FROM runners WHERE entry_id = $1", [
    entryId,
  ]);
...
export async function getRunnerById(id: number): Promise<Runner | null> {
  const db = getDatabase();
  const result = await db.query("SELECT * FROM runners WHERE id = $1", [id]);
...
export async function insertPerformance(
  ...
  try {
    ...
    console.error(
      `insertPerformance: Failed to insert performance for runner_id ${runnerId}:`,
      error
    );
    throw error;
  }
...
export async function insertMatchCandidates(
  runnerId: number,
  ...
  await db.query("DELETE FROM match_candidates WHERE runner_id = $1", [
    runnerId,
  ]);
...
// Team operations
export async function calculateAndSaveTeams(
  ...
  teams.forEach((teamRunners, key) => {
    ...
    const sorted = teamRunners.sort((a, b) => {
      const aVal =
        metric === "all-time"
          ? a.personalBestAllTime
          : a.personalBestLast3Years;
      const bVal =
        metric === "all-time"
          ? b.personalBestAllTime
          : b.personalBestLast3Years;
  ...
  for (const [index, team] of teamData.entries()) {
    // Get runner IDs
    const runner1Id = team.topThree[0]
      ? (
          await db.query("SELECT id FROM runners WHERE entry_id = $1", [
            team.topThree[0].entryId,
          ])
        ).rows[0]?.id
      : null;
    const runner2Id = team.topThree[1]
      ? (
          await db.query("SELECT id FROM runners WHERE entry_id = $1", [
            team.topThree[1].entryId,
          ])
        ).rows[0]?.id
      : null;
    const runner3Id = team.topThree[2]
      ? (
          await db.query("SELECT id FROM runners WHERE entry_id = $1", [
            team.topThree[2].entryId,
          ])
        ).rows[0]?.id
      : null;
...
export async function getTeams(
  ...
  for (const row of result.rows) {
    ...
    const runners = (
      await Promise.all(runnersResult.rows.map((row) => rowToRunner(row)))
    ).sort((a, b) => {
      const aVal =
        metric === "all-time"
          ? a.personalBestAllTime
    ...
    if (row.runner1_id) {
      const r1Result = await db.query("SELECT * FROM runners WHERE id = $1", [
        row.runner1_id,
      ]);
      if (r1Result.rows[0]) topThree.push(await rowToRunner(r1Result.rows[0]));
    }
    if (row.runner2_id) {
      const r2Result = await db.query("SELECT * FROM runners WHERE id = $1", [
        row.runner2_id,
      ]);
      if (r2Result.rows[0]) topThree.push(await rowToRunner(r2Result.rows[0]));
    }
    if (row.runner3_id) {
      const r3Result = await db.query("SELECT * FROM runners WHERE id = $1", [
        row.runner3_id,
      ]);
...
export async function createRunnerNote(
  note: RunnerNoteCreate
): Promise<RunnerNote> {
  const db = getDatabase();
  const result = await db.query(
    `
...
export async function updateRunnerNote(
  id: number,
  updates: RunnerNoteUpdate
): Promise<RunnerNote | null> {
  const db = getDatabase();
  const fields: string[] = [];
  const values: any[] = [];
  let paramIndex = 1;
...
export async function linkRunnersToNews(
  newsId: number,
  runnerIds: number[]
): Promise<void> {
  const db = getDatabase();
  ...
  await db.query(
    "DELETE FROM runner_notes WHERE news_id = $1 AND note_text IS NULL",
    [newsId]
  );
...
export async function updateRaceInfo(
  ...
  if (updates.isActive !== undefined) {
    ...
    if (updates.isActive) {
      await db.query("UPDATE race_info SET is_active = false WHERE id != $1", [
        id,
      ]);
    }
...
export async function getRaceDocuments(
  raceId: number
...
export async function createRaceDocument(
  doc: RaceDocumentCreate
): Promise<RaceDocument> {
  const db = getDatabase();
  const result = await db.query(
    `
...
export async function deleteRaceDocument(id: number): Promise<boolean> {
  const db = getDatabase();
  const result = await db.query("DELETE FROM race_documents WHERE id = $1", [
    id,
  ]);
  return result.rowCount !== null && result.rowCount > 0;
}
